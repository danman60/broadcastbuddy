...
- **Overlay Mode remote toggle** (`d4c2708`): new `toggleOverlayMode` wsHub command (user-noted pending — panels needed a Tools click, couldn't drive over SSH). Headless test: opens panels (win count 1→5) + closes back. Suite 305→306. Lets Stream Deck/script/SSH turn Overlay Mode on/off + lets me verify the panel-freeze fix live.
- `toggleOverlayMode` **VERIFIED LIVE on DART**: sent over WS → log "opening 4 panels" → "closing all panels" clean round-trip. Solves the user's SSH-trigger gap + confirms the panel system + panel-freeze fix work live.
- All pushed `1db3bff`→`576c085`. **Suite 273→306.** DART consolidated at `d4c2708` (llm import fix + overlay-mode toggle live). BB+cast healthy, decoder=0, Overlay Mode off, session intact.
- **Night tally: 9 real bugs fixed + 2 user-flagged items (import conflation, overlay-mode toggle); 5 review passes; full reliability hardening (cast watchdog, OBS reconnect) all live-verified; 306 tests.**
